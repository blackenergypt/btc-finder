...
rl.question(`Escolha uma carteira puzzle( ${chalk.cyan(1)} - ${chalk.cyan(160)}): `, (answer) => {
    ...
    if (Number.parseInt(answer) < 1 || Number.parseInt(answer) > 160) {
    ...
    console.log('Numero possivel de chaves:',  chalk.yellow(Number.parseInt(BigInt(max) - BigInt(min)).toLocaleString('pt-BR')))
    key = BigInt(min)
    ...
    rl.question(`Escolha uma opcao (${chalk.cyan(1)} - Comecar do inicio, ${chalk.cyan(2)} - Escolher uma porcentagem, ${chalk.cyan(3)} - Escolher minimo): `, (answer2) => {
        if (answer2 === '2'){
            rl.question('Escolha um numero entre 0 e 1: ', (answer3) => {
                if (Number.parseFloat(answer3) > 1 || Number.parseFloat(answer3) < 0) {
                ...
                const percentualRange = range * BigInt(Math.floor(Number.parseFloat(answer3) * 1e18)) / BigInt(1e18);
